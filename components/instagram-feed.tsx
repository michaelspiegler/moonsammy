...
export function InstagramFeed({ photos, onPhotoUpdate, user, onUserChange }: InstagramFeedProps) {
  ...
  const [showLikeInput, setShowLikeInput] = useState<{ [key: string]: boolean }>({})
  const [showCommentInput, setShowCommentInput] = useState<{ [key: string]: boolean }>({})
  const [likeNames, setLikeNames] = useState<{ [key: string]: string }>({})
  ...
  useEffect(() => {
    ...
    if (savedName && savedName.trim().length >= 2) {
      setUploaderName(savedName.trim())
    }
  }, [])
  ...
  const getCurrentUserName = () => {
    return localUser ? localUser.name : uploaderName.trim()
  }

  const handleCommentClick = (photoId: string) => {
    const currentName = getCurrentUserName()
    if (!currentName) {
      // Instead of alert, show the comment input with a helpful message
      setShowCommentInput({
        ...showCommentInput,
        [photoId]: true,
      })
  ...
  const handleLikeClick = (photoId: string) => {
    ...
    if (userLike) {
      ...
      if (!currentAuthor) {
        setShowLikeInput({ ...showLikeInput, [photoId]: true })
      } else {
  ...
  const handleLike = async (photoId: string, name: string) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setShowLikeInput({ ...showLikeInput, [photoId]: false })
        setLikeNames({ ...likeNames, [photoId]: "" })
      }
    } catch (error) {
  ...
  return (
    <div className="w-full">
      {/* Show user status if they have a name set or are logged in */}
      {(uploaderName || localUser) && (
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-green-700 font-medium text-sm">
                {localUser ? `${localUser.name} (Account)` : `${uploaderName} (Quick)`}
              </span>
            </div>
        ...
        {filteredPhotos.map((photo) => (
          <div key={photo.id} className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            ...
            <div className="p-3">
              ...
              {showLikeInput[photo.id] && (
                <div className="mb-3 p-2 bg-gray-50 rounded">
                  <div className="flex items-center space-x-2">
                    <Input
                      placeholder="Your name to love this memory"
                      value={likeNames[photo.id] || ""}
                      onChange={(e) => setLikeNames({ ...likeNames, [photo.id]: e.target.value })}
                      className="text-xs h-7"
                      onKeyDown={(e) => {
                        if (e.key === "Enter" && (likeNames[photo.id] || "").trim().length >= 2) {
                          e.preventDefault()
                          handleLike(photo.id, likeNames[photo.id] || "")
                        }
                      }}
                    />
                    <Button
                      onClick={() => handleLike(photo.id, likeNames[photo.id] || "")}
                      disabled={
                        !likeNames[photo.id]?.trim() || likeNames[photo.id].trim().length < 2 || loading[photo.id]
                      }
                      size="sm"
                  ...
                  {currentUserName ? (
                    ...
                    <div className="text-center py-2">
                      <p className="text-sm text-gray-600 mb-2">
                        Set your name in the upload section above to leave comments
                      </p>
                      <Button
